...
export const samplesPivot = (
  ...
  if (filter) { samples = samplesFilter.call(samples, filter) }
  let calc
  const pivot = Array.isArray(cell = parseCell(cell, side))
    ? (calc = true, Cubic.build(side, banner, (mapper(cell, appendIndex), cell)))
    : (calc = false, Pivot.build(side, banner, cell.field, cell.mode))
  const crostab = CrosTab.from(pivot.spread(samples).toJson())
  if (calc && formula) crostab.map(ar => formula.apply(null, ar))
  return crostab
}

const appendIndex = function (cell) { cell.index = cell.field }
