...
import { min, max, NUM_ASC, STR_ASC, NUM_DESC } from '@aryth/comparer';
import * as Mapper from '@vect/vector-mapper';
import { mutate as mutate$2, mapper as mapper$4 } from '@vect/vector-mapper';
...
import { Columns, column } from '@vect/column-getter';
import { iso, init as init$1, draft } from '@vect/matrix-init';
import * as Mapper$1 from '@vect/matrix-mapper';
import { iterate as iterate$4, mapper as mapper$5, mutate as mutate$4 } from '@vect/matrix-mapper';
import * as Transpose from '@vect/matrix-transpose';
import { transpose as transpose$1 } from '@vect/matrix-transpose';
import { mutate as mutate$3 } from '@vect/column-mapper';
...
const scale$2 = (x, lo, lev, min$1, hi) => min((max(x, lo) - lo) * lev + min$1, hi);
/**
...
const fluoVector = function (vec, presets) {
  ...
  const mapper$1 = this !== null && this !== void 0 && this.mutate ? mutate$2 : mapper$4;
...
const scale$1 = (x, lo, lev, min$1, hi) => min((max(x, lo) - lo) * lev + min$1, hi);
/**
 * @typedef {[number,number,number]} Triple
 * @typedef {function(string):string} dye
 * @typedef {{max:string,min:string,na:string,effects?:string[]}} PresetHEX
...
function fluoByRows(mx, config) {
  const context = this,
        mapper$1 = context !== null && context !== void 0 && context.mutate ? mutate$2 : mapper$4;
...
const wind = (keys, values) => zipper$1(keys, values, (k, v) => [k, v]);
...
class CrosTab$1 {
  ...
  setRowBy(r, fn) {
    return mutate$2(this.row(r), fn, this.width), this;
  }
  ...
  map(fn, {
    ...
    return this.boot({
      rows: mapper$5(this.rows, fn, this.height, this.width)
    }, mutate);
  }
  ...
  mapBanner(fn, {
    ...
    return this.boot({
      head: mapper$4(this.head, fn)
    }, mutate);
  }

  mutate(fn) {
    return mutate$4(this.rows, fn, this.height, this.width), this;
  }

  mutateSide(fn) {
    return mutate$2(this.side, fn), this;
  }

  mutateBanner(fn) {
    return mutate$2(this.head, fn), this;
  }
...
function palettCrostab({
  ...
  if (dyed) {
    ...
    space === HEX$1 ? crostab.mutate(hex => {
      ...
      return _mapper = mapper$4(xyz, v => v.toFixed(0).padStart(3)), dyeFactory(xyz)(_mapper);
    });
  }
...
class CrosTab {
  ...
  static init({
    ...
    return new CrosTab(side, head, init$1(side === null || side === void 0 ? void 0 : side.length, head === null || head === void 0 ? void 0 : head.length, (x, y) => func(x, y)), title);
  }
