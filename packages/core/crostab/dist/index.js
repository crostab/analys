...
class Crostab {
  ...
  static init({ side, head, func, title }) {
    return new Crostab(
      side,
      head,
      init(side?.length, head?.length, (x, y) => func(x, y)),
      title
    )
  }
  ...
  setColumn(c, column) { return mutate$1(this.rows, this.coin(c), (_, i) => column[i], this.height), this }
  setColumnBy(c, fn) { return mutate$1(this.rows, this.coin(c), fn, this.height), this }

  map(fn, { mutate = true } = {}) {
    return this.boot({ rows: mapper(this.rows, fn, this.height, this.width) }, mutate)
  }
  mapSide(fn, { mutate = true } = {}) { return this.boot({ side: mapper$1(this.side, fn) }, mutate) }
  mapBanner(fn, { mutate = true } = {}) { return this.boot({ head: mapper$1(this.head, fn) }, mutate) }
  mutate(fn) { return mutate$2(this.rows, fn, this.height, this.width), this }
  ...
  pushColumn(label, col) { return this.head.push(label), push(this.rows, col), this }
  unshiftColumn(label, col) { return this.head.unshift(label), unshift(this.rows, col), this }
