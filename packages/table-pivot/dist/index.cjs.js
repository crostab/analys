...
const pivotDev = (table, {
  ...
  let pivotter;
  const pivot$1 = Array.isArray(cell = tablespec.parseCell(cell, side)) ? (pivotter = true, cubic.Cubic.build(x, y, (Mapper.iterate(cell, appendIndex.bind(head)), cell))) : (pivotter = false, pivot.Pivot.build(x, y, head.indexOf(cell.field), cell.mode));
...
let deVe = function (vector, lv) {
  Mapper.mutate(vector, v => String(deNode.call(this, v, lv + 1)));
...
const transpose = (mx, h, w) => {
  ...
  for (--w; w >= 0; w--) cols[w] = Mapper.mapper(mx, r => r[w], h);
...
const columns = function (y, h) {
  return Mapper.mapper(this, r => r[y], h);
};
...
const Formatter = (space, color = false) => {
  ...
  if (space === RGB || space === HSL) {
    const formatter = rgb => Mapper.mapper(rgb, x => String(round$1(x)).padStart(3));
...
function VectorAverage(space) {
  if (space === RGB || space === HSL) return Indicator({
    ...
    pick: (rgb, l) => Mapper.mutate(rgb, x => round$1(x / l), 3)
  });
...
function degreesByColors({
  ...
  const columns = Mapper.mapper(selectValues(Cards, colors), card => Mapper.mapper(selectValues(card, degrees), colorPicker, h), w);
  const side = degrees,
        head = Mapper.mapper(colors, readify),
        rows = transpose(columns),
        crostab$1 = new crostab.CrosTab(side, head, mapper$2(rows, formatter));
  ...
  if (average) {
    ...
    crostab$1.unshiftRow(AVERAGE, mapper$3(rows, stat_format), w).unshiftColumn(AVERAGE, [''].concat(Mapper.mapper(rows, stat_format, h)));
  }
  ...
  constructor(roster, effects) {
    ...
    return new Proxy(this, {
      ...
      get(tar, p) {
        ...
        if (p in tar) return typeof (p = tar[p]) === enums.FUN ? p.bind(tar) : p;
...
const makeBand = function (fieldSet) {
  const head = this;
  return Mapper.mapper(fieldSet, ([field, mode]) => ({
