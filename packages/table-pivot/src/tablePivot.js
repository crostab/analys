...
const parseCell = (cell, defaultField) => {
  switch (typeof cell) {
    case OBJ:
      if (Array.isArray(cell)) return cell.length ? cell : { field: defaultField, mode: COUNT }
    ...
    default:
      return { field: defaultField, mode: COUNT }
  }
}
/**
 *
...
export const tablePivot = (table, {
  ...
  // ({ x, y }) |> delogger
  let calc, z
  const pivot = Array.isArray(cell)
    ? (iterate(cell, c => c.index = head.indexOf(c.field)), calc = true, Cubic.build(x, y, cell))
    : (z = head.indexOf(cell.field), calc = false, Pivot.build(x, y, z, cell.mode))
