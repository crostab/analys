...
const samplesPivot = function ({
  ...
  let cubic;
  const crostabEngine = Array.isArray(field = parseField(field, side)) ? (cubic = true, new Cubic(side, banner, field)) : (cubic = false, new Pivot(side, banner, field[0], field[1]));
  const crostab = crostabEngine.record(samples).toObject();
  if (cubic && formula) mutate(crostab.rows, vec => formula.apply(null, vec));
  return crostab;
};
