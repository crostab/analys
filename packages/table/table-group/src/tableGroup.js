...
export const tableGroup = function ({
  ...
  ([{ key, to }] = parseField(key))
  const groupingEngine = (field = parseField.call({ key }, field)).length > 1 // field |> deco |> says['parsed field']
    ? (groupHead = acquire([key], field.map(({ key: label }) => label)),
        new Group(
          [head.indexOf(key), to],
          field.map(({ key: label, to: mode }) => [head.indexOf(label), mode]))
    )
    : (([{ key: label, to: mode }] = field), groupHead = [key, label],
        new Chips(
          [head.indexOf(key), to],
          [head.indexOf(label), mode])
    )
  if (alias)
    for (let [field, proj] of (Array.isArray(alias) ? alias : Object.entries(alias))) {
