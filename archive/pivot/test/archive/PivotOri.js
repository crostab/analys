...
import { acid, ampliCell, arid, modeToInit, Accumulators } from 'archive/util-pivot'
...
const { merge, incre } = Accumulators
export const Updater = function (data, mode) {
  const ri = arid.bind(data), ci = acid.bind(data)
  if (mode === MERGE) return function (x, y, value) { return merge(data.rows[ri(this.sidePick(x))][ci(this.headPick(y))], value) }
  if (mode === ACCUM) return function (x, y, value) { return incre(data.rows[ri(this.sidePick(x))][ci(this.headPick(y))], value) }
